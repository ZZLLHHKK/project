...
class ActionCodec:
    """Codec for action dict/dataclass and legacy actions.txt lines."""

    def action_to_dict(self, action: Union[Action, ActionDict]) -> ActionDict:
        if isinstance(action, dict):
            return dict(action)
        return asdict(action)

    def dict_to_action(self, d: ActionDict) -> ActionDict:
        # Keep as dict (LangGraph-friendly). Validator will normalize.
        return dict(d)

    def action_to_line(self, action: Union[Action, ActionDict]) -> str:
        d = self.action_to_dict(action)
    ...
    def actions_to_text(self, actions: List[Union[Action, ActionDict]]) -> str:
        return "\n".join(self.action_to_line(a) for a in actions) + ("\n" if actions else "")

    def parse_action_line(self, line: str) -> Optional[ActionDict]:
        """
        ...
        # Extract optional DURATION=xx token.
        duration = None
        kept = []
        for p in parts:
...
def action_to_dict(action: Union[Action, ActionDict]) -> ActionDict:
    ...
    return DEFAULT_ACTION_CODEC.action_to_dict(action)


def dict_to_action(d: ActionDict) -> ActionDict:
    """Backward-compatible function wrapper."""
    return DEFAULT_ACTION_CODEC.dict_to_action(d)


def action_to_line(action: Union[Action, ActionDict]) -> str:
    """Backward-compatible function wrapper."""
    return DEFAULT_ACTION_CODEC.action_to_line(action)


def actions_to_text(actions: List[Union[Action, ActionDict]]) -> str:
    """Backward-compatible function wrapper."""
    return DEFAULT_ACTION_CODEC.actions_to_text(actions)


def parse_action_line(line: str) -> Optional[ActionDict]:
    """Backward-compatible function wrapper."""
    return DEFAULT_ACTION_CODEC.parse_action_line(line)
