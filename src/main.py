...
def print_dashboard(state: StateManager):
    ...
    print(f"💡 燈光狀態: 客廳({state.led_states['LIVING']}) | 廚房({state.led_states['KITCHEN']}) | 客房({state.led_states['GUEST']})")
    print("="*40 + "\n")
...
def main():
    ...
    while True:
        try:
            user_input = input("🗣️ 請輸入您的指令 (輸入 'exit' 退出): ")
